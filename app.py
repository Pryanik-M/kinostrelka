...
def index():
    ...
    new_movies = Movie.query.order_by(Movie.id.desc()).limit(8).all()

    # Рекомендуемые фильмы (исключая новинки)
    new_movie_ids = [movie.id for movie in new_movies]
    recommended_movies = Movie.query.filter(
        ~Movie.id.in_(new_movie_ids)
    ).order_by(db.func.random()).limit(8).all()

    if current_user.is_authenticated and current_user.email == 'matveypryanikov@yandex.ru':
        return render_template('index.html', flag=flag_user,
                               new_movies=new_movies,
                               recommended_movies=recommended_movies,
                               comments=comments, admin=True)
    return render_template('index.html', flag=flag_user,
                           new_movies=new_movies,
                           recommended_movies=recommended_movies,
                           comments=comments)
...
def movie_page():
    ...
    poster_path = os.path.join('static', 'posters', movie.poster_img)
    if not os.path.exists(poster_path):
        poster = 'default.jpg'
    else:
        poster = movie.poster_img
...
@login_required
def admin_page():
    if current_user.email != 'matveypryanikov@yandex.ru':
        return render_template('index.html', error=True)

    if request.method == 'POST':
        # Создаем папку для постеров если ее нет
        posters_dir = os.path.join('static', 'posters')
        os.makedirs(posters_dir, exist_ok=True)

        # Обрабатываем загрузку файла
        if 'poster' not in request.files:
            return render_template('admin.html', error='Файл постера не загружен')

        file = request.files['poster']
        if file.filename == '':
            return render_template('admin.html', error='Файл не выбран')

        # Генерируем уникальное имя файла
        if file:
            ext = file.filename.split('.')[-1]
            filename = f"{uuid.uuid4().hex}.{ext}"
            file.save(os.path.join(posters_dir, filename))
        else:
            return render_template('admin.html', error='Ошибка загрузки файла')

        # Создаем запись в базе данных
        new_movie = Movie(
            name=request.form['name'],
            poster_img=filename,  # Сохраняем только имя файла
    ...
    return render_template('admin.html', access_denied=False)
...
if __name__ == '__main__':
    app.run(host='192.168.31.89', port=5000, debug=True)